<template>
 <q-input
    ...
    stack-label
    @focusout.prevent="setDefaultValue"
    >
...
<script setup>
import {ref, toRef, isRef, isReactive, reactive, defineEmits, onMounted, computed} from 'vue'
...
onMounted(()=>  setValueScore())
const setValueScore = () => {
  if (!props.wk) return


   score = toRef($store.activeWorkouts.find((wk)=>wk.workout_id == props.workoutId).results.find((exercise)=>exercise.wk_exercise_id == props.wkExerciseId).workout_fields.setRow[props.indexRow].fields[props.indexField].score)
